#!/usr/bin/env python
"""
Document Collection Script for Limnos.

This script provides a command-line interface for the Universal Document Collector,
allowing users to collect documents from files or directories and store them with
universal metadata.

Usage:
    python collect_documents.py collect-file --file_path /path/to/document.pdf
    python collect_documents.py collect-dir --dir_path /path/to/directory --recursive
    python collect_documents.py list
    python collect_documents.py get --doc_id <document_id>
"""
...
def init_collector(source_dir=None):
...
def collect_file(args):
    ...
    collector = init_collector(args.source_dir)
    ...
    metadata_preview = {k: v for k, v in document.metadata.items() 
                      if k in ['title', 'authors', 'abstract', 'doc_type', 'structure']}
...
def collect_directory(args):
    ...
    for document, paths in results:
        print(f"  - {document.metadata.get('title', 'Unknown')} (ID: {document.metadata['doc_id']})")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Limnos Document Collection Tool')
